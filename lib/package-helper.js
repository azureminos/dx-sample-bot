...
const dummyInstance = ({
  ...
  _.each(packageItems, (item) => {
    console.log(
      `Dummy PackageItem id[${
        item.attraction ? item.attraction.id : null
      }] vs _id[${item.attraction ? item.attraction._id : null}]`,
      item.attraction
    );
    const it = {
      id: getUniqueId(),
  ...
  _.each(packageHotels, (item) => {
    console.log(
      `Dummy PackageHotel id[${item.hotel ? item.hotel.id : null}] vs _id[${
        item.hotel ? item.hotel._id : null
      }]`,
      item.hotel
    );
    const ho = {
