...
const getCarRate = (params, carRates) => {
  ...
  for (let i = 0; carRates && i < carRates.length; i++) {
    const cr = carRates[i];
    const city = cr.name;
    // Check if match city
    for (let m = 0; cr.carRates && m < cr.carRates.length; m++) {
...
exports.calFlightRate = (startDate, flightRates) => {
  if (startDate) {
    for (let i = 0; i < flightRates.length; i++) {
...
exports.calItemRate = (params, items) => {
  ...
  for (let i = 0; tmpItems && i < tmpItems.length; i++) {
...
exports.calHotelRate = (params, hotels) => {
  ...
  for (let i = 0; tmpHotels && i < tmpHotels.length; i++) {
